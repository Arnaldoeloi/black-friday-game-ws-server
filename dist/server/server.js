...
function getGameByCode(serverCode) {
    console.log(games === null || games === void 0 ? void 0 : games.find((game) => game.serverCode == serverCode));
    return games === null || games === void 0 ? void 0 : games.find((game) => game.serverCode == serverCode);
}
function notifyPrivateDataToAllPlayers(game) {
    game.players.forEach((p) => notifyPrivateDataToPlayer(p));
}
function notifyPrivateDataToPlayer(player) {
...
function broadcastEventToAllPlayers(event, game) {
...
wss.on('connection', (ws, request, client) => {
    ...
    ws.on('dropBomb', function (data) {
        ...
        if (game) {
            if (game.state == Game_1.GAME_STATE.GAME_ENDED) {
                const event = new Event('error', { 'message': 'The game already ended!' });
                ws.send(event.toString());
                return false;
            }
            if (game.state != Game_1.GAME_STATE.WAITING_BOMB_DROP) {
                const event = new Event('error', { 'message': 'The game is not on this state.' });
                ws.send(event.toString());
                return false;
            }
            if (((_a = game.turnOwner) === null || _a === void 0 ? void 0 : _a.name) != clientName) {
            ...
            if (clientName) {
                ...
                if (player && adversary) {
                    ...
                    // }
                    const event = new Event('bombDropped', Object.assign(Object.assign({}, game.publicData()), { 'bomb': bombStatus, 'droppedOnPlayer': adversary.name }));
        ...
        if (game) {
            if (clientName) {
                ...
                if (player) {
                    ...
                    if (game.allPlayersReady()) {
                        console.log('All players ready, changing game state');
                        game.state = Game_1.GAME_STATE.WAITING_BOMB_DROP;
                        game.setRandomTurnOwner();
                        // notifyPrivateDataToAllPlayers(game);
                    }
